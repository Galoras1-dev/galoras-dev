...
export function Navbar() {
  ...
  const checkRoles = async (userId: string) => {
    ...
    // A user can be BOTH a coach and a client. "Is a coach" = they have a coach
    // profile (a row in `coaches`). Everyone can also act as a client/coachee, so
    // dual-role users get a view switcher rather than being locked to one side.
          ...
          <div className="hidden lg:flex items-center gap-3">
            {!isCoach && (
              <Button variant="ghost" size="sm" asChild>
                <Link to="/apply">Join as a Coach</Link>
              </Button>
            )}
            {isLoggedIn ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  {/* Dual-role view switcher: a user with a coach profile sees both
                      "Coach Dashboard" and "Client Dashboard" and flips between them.
                      FUTURE: a single-role user who wants the other role flips here into
                      an upgrade journey — coachee -> coach via /apply (application +
                      onboarding + tier purchase); coach -> coachee can sit behind the
                      client paywall when the coachee side is monetised. */}
                  {isCoach ? (
                    <>
